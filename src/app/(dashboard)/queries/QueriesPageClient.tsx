...
import { ShieldAlert, Activity, Lock, Terminal, ShieldCheck, Wifi, HelpCircle, X, Check, AlertTriangle } from "lucide-react";
...
export default function QueriesPageClient({ 
    ...
    const [selectedToolHelp, setSelectedToolHelp] = useState<TooltipDetails | null>(null);

    const helpData: Record<string, TooltipDetails> = {
        firewall: {
    ...
    return (
        <div className="page-container">
                    ...
                    >
                        ...
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                e.preventDefault();
                                const idKey = tool.id === 'hibp-account' ? 'hibp-account' : tool.id === 'hibp-domain' ? 'hibp-domain' : tool.id;
                                if (helpData[idKey]) {
                                    setSelectedToolHelp(helpData[idKey]);
                                }
                            }}
                            style={{
                                position: 'absolute',
            ...
            {selectedToolHelp && (
                ...
                onClick={() => setSelectedToolHelp(null)}
                >
                    <div style={{
                        background: '#121214',
                        border: '1px solid var(--border-color)',
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxHeight: '70vh', overflowY: 'auto', paddingRight: '4px' }}>
                            <div>
                                ...
                                <ul style={{ margin: 0, paddingLeft: '20px', color: 'var(--text-primary)', fontSize: '0.925rem', display: 'flex', flexDirection: 'column', gap: '6px', lineHeight: 1.5 }}>
                                    {selectedToolHelp.capabilities.map((cap, i) => (
                                        <li key={i}>{cap}</li>
                                    ))}
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    {selectedToolHelp.colors.map((color, i) => (
                            ...
                            {selectedToolHelp.backgroundJobs && (
                                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                                    ...
                                    <ul style={{ margin: 0, paddingLeft: '20px', color: 'var(--text-muted)', fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '4px', lineHeight: 1.4 }}>
                                        {selectedToolHelp.backgroundJobs.map((job, i) => (
                                            <li key={i}>{job}</li>
                                        ))}
                                    </ul>
                                </div>
