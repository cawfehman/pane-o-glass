...
if (!targetPostgresUrl || (!targetPostgresUrl.startsWith("postgres://") && !targetPostgresUrl.startsWith("postgresql://"))) {
    ...
    console.error("Example: DATABASE_URL=\"postgresql://pane_user:Password123!@172.16.x.x:5432/pane_o_glass?schema=public\"");
    process.exit(1);
}

// Initialize SQLite Client pointing explicitly to SQLite dev.db file
...
// Initialize Target PostgreSQL Client
const postgresClient = new SqliteClient({
    datasources: {
        db: {
...
async function migrateData() {
    ...
    try {
        ...
        // 7. Migrate Guardian Events & Blacklist
