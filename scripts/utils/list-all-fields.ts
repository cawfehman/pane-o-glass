...
async function listAllFields() {
    ...
    try {
        ...
        const wirelessSession = macMatches.find((m: any) => m.includes('CooperEmployee')) || macMatches[0];
        ...
        const uniqueTags = [...new Set(tags.map((t: any) => t.replace(/[<>/\s]/g, '').split(' ')[0]))].filter((t: any) => t && t !== '?xml' && !t.startsWith('!'));
